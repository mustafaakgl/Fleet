...
export default function DepartureChecksPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setChecks([]);
      setMissing([]);
      setError(getApiErrorMessage(e, t('departureChecks.loadError')));
    } finally {
      setLoading(false);
    }
  }, [t]);
    ...
    <div className={FLEET_PAGE}>
      <div className={`${FLEET_PAGE_HEADER} flex flex-wrap items-center justify-between gap-3`}>
        <div className="flex items-center gap-3">
          ...
          <h1 className={FLEET_PAGE_TITLE}>{t('departureChecks.title')}</h1>
        </div>
        <Button variant="outline" asChild>
          <Link href="/defects">{t('nav.defects')}</Link>
        </Button>
      </div>
      ...
      {!loading && error ? (
        <EmptyState
          icon={WifiOff}
          title={t('departureChecks.loadErrorTitle')}
          subtitle={error}
        ...
        <Card className="border-amber-200 bg-amber-50/50">
          <CardHeader className="pb-2">
            <CardTitle className="text-base text-amber-900">
              {t('departureChecks.missingToday')} ({missing.length})
            </CardTitle>
          </CardHeader>
      ...
      <Card className={FLEET_LIST_CARD}>
        {loading ? (
          <div className="p-6 text-center text-sm text-slate-500">{t('departureChecks.loading')}</div>
        ) : !error && checks.length === 0 ? (
          <EmptyState
            icon={ClipboardCheck}
            title={t('departureChecks.emptyTitle')}
            subtitle={t('departureChecks.emptyMessage')}
          />
        ) : !error ? (
          <Table className={FLEET_TABLE}>
            <TableHeader>
              <TableRow className={FLEET_TABLE_HEADER_ROW}>
                <TableHead className={FLEET_TABLE_HEAD}>{t('departureChecks.colDate')}</TableHead>
                <TableHead className={FLEET_TABLE_HEAD}>{t('departureChecks.colDriver')}</TableHead>
                <TableHead className={FLEET_TABLE_HEAD}>{t('departureChecks.colVehicle')}</TableHead>
                <TableHead className={FLEET_TABLE_HEAD}>{t('departureChecks.colStatus')}</TableHead>
              </TableRow>
            </TableHeader>
