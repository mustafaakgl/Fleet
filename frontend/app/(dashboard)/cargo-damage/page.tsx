  ...
  return (
    <div className="space-y-4 sm:space-y-5">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-xl font-bold text-gray-900 sm:text-2xl">{t('cargoDamage.title')}</h1>
        {!loading && !error && (
        ...
        {loading ? (
          ...
          <div className={FLEET_LIST_MOBILE}>
            {visibleReports.map((r) => (
              <MobileDataCard
                key={r.id}
                title={r.cargoName ?? t('cargoDamage.colCargoName')}
                subtitle={`${r.driver ? `${r.driver.firstName} ${r.driver.lastName}` : '—'} · ${formatDate(r.incidentDateTime)}`}
                badge={
