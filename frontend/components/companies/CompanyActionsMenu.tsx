...
export function CompanyActionsMenu({ canImport, onImport, onExport }: CompanyActionsMenuProps) {
  ...
  return (
    <div ref={rootRef} className="relative">
      <Button
        ...
        size="icon"
        className="h-10 w-10"
        aria-label={t('companies.moreActions', { defaultValue: 'More actions' })}
        aria-expanded={open}
      ...
      {open ? (
        <div className="absolute right-0 z-50 mt-2 w-64 overflow-hidden rounded-md border border-slate-200 bg-white py-1 shadow-lg">
          {canImport ? (
            ...
            >
              <span>{t('companies.import.menuLabel', { defaultValue: 'Import' })}</span>
          ...
          >
            <span>{t('companies.export.menuLabel', { defaultValue: 'Export Data' })}</span>
