...
export function DriverImportDialog({ open, onClose, onImported }: DriverImportDialogProps) {
  ...
  async function handleImport() {
    ...
    setError(null);
    try {
      const importResult = await importApi.drivers(file);
      ...
      setError(
        err instanceof Error
          ? err.message
          : t('drivers.import.error', { defaultValue: 'Could not import drivers.' }),
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    ...
    >
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>{t('drivers.import.title', { defaultValue: 'Import Driver' })}</DialogTitle>
        </DialogHeader>

        <div className="space-y-4">
          <p className="text-sm text-slate-600">
            {t('drivers.import.hint', {
              defaultValue:
                'Upload a CSV file with driver rows. First name and last name are required.',
            })}
          </p>
          ...
          >
            ...
            {t('drivers.import.downloadSample', { defaultValue: 'Download sample CSV' })}
          </a>
          ...
          >
            ...
            <p className="text-sm font-medium text-slate-700">
              {t('drivers.import.dropLabel', { defaultValue: 'Drop CSV here or click to browse' })}
            </p>
            <p className="mt-1 text-xs text-slate-500">
              {t('drivers.import.dropHint', { defaultValue: '.csv files only' })}
            </p>
            <input
              ref={inputRef}
              type="file"
        ...
        <DialogFooter>
          ...
          {!result ? (
            <Button type="button" disabled={!file || loading} onClick={() => void handleImport()}>
              ...
              {loading
                ? t('import.uploading')
                : t('drivers.import.upload', { defaultValue: 'Import CSV' })}
            </Button>
          ) : null}
        </DialogFooter>
      </DialogContent>
