...
export function VehicleImportDialog({ open, onClose, onImported }: VehicleImportDialogProps) {
  ...
  async function handleImport() {
    ...
    setError(null);
    try {
      const importResult = await importApi.vehicles(file);
      ...
      setError(
        err instanceof Error
          ? err.message
          : t('vehicles.import.error', { defaultValue: 'Could not import vehicles.' }),
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    ...
    >
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>{t('vehicles.import.title', { defaultValue: 'Import Vehicle' })}</DialogTitle>
        </DialogHeader>

        <div className="space-y-4">
          <p className="text-sm text-slate-600">
            {t('vehicles.import.hint', {
              defaultValue:
                'Upload a CSV file with vehicle rows. Plate number, brand, and model are required.',
            })}
          </p>
          ...
          >
            ...
            {t('vehicles.import.downloadSample', { defaultValue: 'Download sample CSV' })}
          </a>
          ...
          >
            ...
            <p className="text-sm font-medium text-slate-700">
              {t('vehicles.import.dropLabel', { defaultValue: 'Drop CSV here or click to browse' })}
            </p>
            <p className="mt-1 text-xs text-slate-500">
              {t('vehicles.import.dropHint', { defaultValue: '.csv files only' })}
            </p>
            <input
              ref={inputRef}
              type="file"
          ...
          {file ? (
            <p className="text-sm text-slate-700">
              {t('vehicles.import.selectedFile', {
                name: file.name,
        ...
        <DialogFooter>
          ...
          {!result ? (
            <Button type="button" disabled={!file || loading} onClick={() => void handleImport()}>
              ...
              {loading
                ? t('import.uploading')
                : t('vehicles.import.upload', { defaultValue: 'Import CSV' })}
            </Button>
          ) : null}
        </DialogFooter>
      </DialogContent>
