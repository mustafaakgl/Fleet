...
import { CloudDownload, Ellipsis } from 'lucide-react';
import { useTranslation } from 'react-i18next';
import { Button } from '@/components/ui/button';
import { cn } from '@/lib/utils';

interface MessengerActionsMenuProps {
  onExport: () => void;
  exporting?: boolean;
}

export function MessengerActionsMenu({ onExport, exporting }: MessengerActionsMenuProps) {
  ...
  return (
    <div ref={rootRef} className="relative">
      <Button
        ...
        size="icon"
        className="h-10 w-10"
        aria-label={t('expenseHistory.moreActions')}
        aria-expanded={open}
      ...
      {open ? (
        <div className="absolute right-0 z-50 mt-2 w-56 overflow-hidden rounded-md border border-slate-200 bg-white py-1 shadow-lg">
          ...
          >
            <span>{t('messenger.export.menuLabel')}</span>
