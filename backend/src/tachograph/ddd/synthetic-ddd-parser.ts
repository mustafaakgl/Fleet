...
const logger = new Logger('SyntheticDddParser');
...
export class SyntheticDddParser implements DddParserPort {
  parse(buffer: Buffer): ParsedDddResult {
    const warnings: string[] = [];

    if (!buffer || buffer.length < 12) {
      return emptyParsedDddResult(['File is too small to be a valid DDD payload.']);
    }
    ...
    try {
      ...
      if (tlvs.length === 0) {
        return emptyParsedDddResult(['No TLV/TREP blocks found.']);
      }
      ...
      logger.warn(`Synthetic DDD parse failed: ${message}`);
      return emptyParsedDddResult([`Parser error: ${message}`]);
    }
  }
}
